...
for(const match of html.matchAll(/<script\b[^>]*src="([^"]+)"[^>]*><\/script>/g))scripts.push(await readFile('public/'+match[1],'utf8'));
html=html.replace(/<script\b[^>]*src="[^"]+"[^>]*><\/script>/g,'');
for(const match of [...html.matchAll(/<link rel="stylesheet" href="([^"]+)">/g)])html=html.replace(match[0],'<style>'+await readFile('public/'+match[1],'utf8')+'</style>');
...
await mkdir('exports',{recursive:true});await writeFile('exports/Saricicek-Family.html',html);console.log('Single-file interactive preview: exports/Saricicek-Family.html. Demo data only; maps require internet.');
